...
-- Actualizar datos en una entrada
UPDATE entries
	SET 
...
--Buscar todos y que muestre los datos del autor y sin ID de la entry:
SELECT e.title,e.content,e.date,e.category,a.name,a.surname,a.image
FROM entries AS e
INNER JOIN authors AS a
ON e.id_author=a.id_author;
...
DELETE
FROM entries
WHERE entries.title ='En busca de Boby Fisher 2';

-- Recibir todos los datos de authors
  SELECT * 
  FROM authors;

-- Recibir los datos de un author por su email
  SELECT 
	au.id_author, 
	au.name, 
	au.surname, 
	...
	au.image
FROM authors AS au
WHERE au.email = 'alejandru@thebridgeschool.es';

--Actualizar datos en un autor por email
UPDATE authors
	SET 
    name='Rían', 
    surname ='Rodríguez', 
    id_author=(SELECT id_author FROM authors WHERE email='rrodriguez@hotmail.es'), 
    image='https://i.pinimg.com/236x/99/af/a2/99afa23e6077c877375debf0613beb63.jpg'
	WHERE email='rrodriguez@hotmail.es';

  --Borrar un autor por su email
  DELETE
  FROM authors
  WHERE authors.email= 'sergiolillom@hotmail.com';